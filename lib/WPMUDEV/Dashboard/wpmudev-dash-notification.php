...
if ( ! class_exists( 'PSOURCE_Dashboard_Notice4' ) ) {
	class PSOURCE_Dashboard_Notice4 {
		...
		var $theme_pack = 128;
		var $server_url = 'https://premium.psource.org/api/dashboard/v1/';
		var $update_count = 0;
		...
		function remove_older() {
			global $PSOURCE_Dashboard_Notice3;

			//remove 3.0 notices
			if ( is_object( $PSOURCE_Dashboard_Notice3 ) ) {
				remove_action( 'init', array( $PSOURCE_Dashboard_Notice3, 'init' ) );
				remove_action( 'plugins_loaded', array( $PSOURCE_Dashboard_Notice3, 'init' ) );
			} else if ( method_exists( 'PSOURCE_Dashboard_Notice3', 'init' ) ) { //if class is not in global (some projects included inside a method), we have to use a hacky way to remove the filter
				$this->deregister_hook( 'init', 'PSOURCE_Dashboard_Notice3', 'init', 10 );
				$this->deregister_hook( 'plugins_loaded', 'PSOURCE_Dashboard_Notice3', 'init', 10 );
			}

			//remove version 2.0
			if ( method_exists( 'PSOURCE_Dashboard_Notice', 'init' ) ) {
				$this->deregister_hook( 'init', 'PSOURCE_Dashboard_Notice', 'init', 10 );
				$this->deregister_hook( 'plugins_loaded', 'PSOURCE_Dashboard_Notice', 'init', 10 );
			}
		...
		function init() {
			...
			if ( class_exists( 'PSOURCE_Dashboard' ) || ( isset( $psource_un->version ) && version_compare( $psource_un->version, '3.4', '<' ) ) ) {
				return;
			}

			// Schedule update cron on main site only
			if ( is_main_site() ) {
				if ( ! wp_next_scheduled( 'psource_scheduled_jobs' ) ) {
					wp_schedule_event( time(), 'twicedaily', 'psource_scheduled_jobs' );
				}

				add_action( 'psource_scheduled_jobs', array( $this, 'updates_check' ) );
			}
			...
			if ( is_admin() && current_user_can( 'install_plugins' ) ) {
				...
				if ( file_exists( WP_PLUGIN_DIR . '/psource-updates/update-notifications.php' ) ) {
					if ( ! get_site_option( 'wdp_un_autoactivated' ) ) {
						//include plugin API if necessary
						if ( ! function_exists( 'activate_plugin' ) ) {
							require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
						}
						$result = activate_plugin( '/psource-updates/update-notifications.php', network_admin_url( 'admin.php?page=psource' ), is_multisite() );
		...
		function is_allowed_screen() {
			...
			if ( isset( $psource_notices ) && is_array( $psource_notices ) ) {
				foreach ( $psource_notices as $product ) {
		...
		function auto_install_url() {
			...
			return wp_nonce_url( $function( "update.php?action=install-plugin&plugin=install_psource_dash" ), "install-plugin_install_psource_dash" );
		}

		function activate_url() {
			$function = is_multisite() ? 'network_admin_url' : 'admin_url';

			return wp_nonce_url( $function( 'plugins.php?action=activate&plugin=psource-updates%2Fupdate-notifications.php' ), 'activate-plugin_psource-updates/update-notifications.php' );
		}
		...
		function install_notice() {
			...
			<input type="hidden" name="msg_id" value="<?php _e( 'install', 'psource' ); ?>" />
			...
			<div class="wdpun-notice-message">
				...
				if ( $this->product_name ) {
					if ( $this->product_update ) {
						printf( __( 'Important updates are available for <strong>%s</strong>. Install the free WPMU DEV Dashboard plugin now for updates and support!', 'psource' ), esc_html( $this->product_name ) );
					} else {
						printf( __( '<strong>%s</strong> is almost ready - install the free WPMU DEV Dashboard plugin for updates and support!', 'psource' ), esc_html( $this->product_name ) );
					}

				} else if ( $this->update_count ) {
					_e( 'Important updates are available for your WPMU DEV plugins/themes. Install the free WPMU DEV Dashboard plugin now for updates and support!', 'psource' );
				} else {
					_e( 'Almost ready - install the free WPMU DEV Dashboard plugin for updates and support!', 'psource' );
				}
				?>
			</div><!-- end wdpun-notice-message -->
			<div class="wdpun-notice-cta">
				<a href="<?php echo $this->auto_install_url(); ?>"
				   class="wdpun-button wdpun-button-small"><?php _e( 'Install Plugin', 'psource' ); ?></a>

				<a class="wdpun-button wdpun-button-notice-dismiss"
				        data-msg="<?php _e( 'Saving...', 'psource' ); ?>">
					<?php _e( 'Dismiss', 'psource' ); ?>
				</a>
		...
		function activate_notice() {
			...
			<input type="hidden" name="msg_id" value="<?php _e( 'activate', 'psource' ); ?>" />
			...
			<div class="wdpun-notice-message">
				...
				if ( $this->product_name ) {
					if ( $this->product_update ) {
						printf( __( 'Important updates are available for <strong>%s</strong>. Activate the WPMU DEV Dashboard to update now!', 'psource' ), esc_html( $this->product_name ) );
					} else {
						printf( __( 'Just one more step to enable updates and support for <strong>%s</strong>!', 'psource' ), esc_html( $this->product_name ) );
					}

				} else if ( $this->update_count ) {
					_e( 'Important updates are available for your WPMU DEV plugins/themes. Activate the WPMU DEV Dashboard to update now!', 'psource' );
				} else {
					_e( "Just one more step - activate the WPMU DEV Dashboard plugin and you're all done!", 'psource' );
				}
				?>
			</div><!-- end wdpun-notice-message -->
			<div class="wdpun-notice-cta">
				<a href="<?php echo $this->activate_url(); ?>"
				   class="wdpun-button wdpun-button-small"><?php _e( 'Activate WPMU DEV Dashboard', 'psource' ); ?></a>
		...
		function updates_check() {
			...
			foreach ( $local_projects as $pid => $item ) {
				 ...
				 *
				 * @since  4.0.0
				 * @api    psource_api_project_extra_data-$pid
				 *
				 * @param  string $extra Default extra data is an empty string.
				 */
				$extra = apply_filters( "psource_api_project_extra_data-$pid", $extra );
				$extra = apply_filters( 'psource_api_project_extra_data', $extra, $pid );
		...
		function filter_plugin_info( $res, $action, $args ) {
			...
			if ( ( $action == 'plugin_information' || $action == 'theme_information' ) && strpos( $args->slug, 'psource_install' ) !== false ) {
			...
			if ( $action == 'plugin_information' && strpos( $args->slug, 'install_psource_dash' ) !== false ) {
				$res                = new stdClass;
				$res->name          = 'WPMU DEV Dashboard';
				$res->slug          = 'wpmu-dev-dashboard';
				$res->version       = '';
				$res->rating        = 100;
				$res->homepage      = 'https://premium.psource.org/project/wpmu-dev-dashboard/';
			...
			if ( is_array( $updates ) && count( $updates ) ) {
				foreach ( $updates as $id => $plugin ) {
					if ( $plugin['type'] != 'theme' && $plugin['autoupdate'] != '2' ) {
						...
						$object->slug        = "psource_install-$id";
		...
		function popup_changelog( $project_id ) {
			...
			if ( ! $changelog || ! is_array( $changelog ) || ! $item ) {
				wp_die( __( 'We did not find any data for this plugin or theme...', 'psource' ) );
			}
			...
			<div id="content" class="<?php echo esc_attr( $dlg_id ); ?>">
				...
				<div class="excerpt">
					<h1><?php printf( esc_attr__( '%s changelog', 'psource' ), esc_html( $item['name'] ) ); ?></h1>
				</div>
					...
					<ul class="versions">
						...
						foreach ( $changelog as $log ) {
							...
							printf(
								...
								esc_attr( $row_class ),
								sprintf(
									esc_html__( 'Version %s', 'psource' ), esc_html( $version )
								),
								wp_kses_post( $badge ),
								esc_html( $rel_date )
							);
							...
							foreach ( $notes as $note ) {
								if ( 0 === strpos( $note, '<p>' ) ) {
									...
									<a href="#" class="for-intro">%s</a><a href="#" class="for-detail">%s</a>
											...
											esc_html__( 'Show all changes', 'psource' ),
											esc_html__( 'Hide details', 'psource' )
										);
										$detail_class = 'detail';
									}
									$detail_level += 1;
	...
	$GLOBALS['PSOURCE_Dashboard_Notice4'] = new PSOURCE_Dashboard_Notice4();
}

//disable older versions
if ( ! class_exists( 'PSOURCE_Dashboard_Notice' ) ) {
	class PSOURCE_Dashboard_Notice {
	}
}

if ( ! class_exists( 'PSOURCE_Dashboard_Notice3' ) ) {
	class PSOURCE_Dashboard_Notice3 {
	}
}