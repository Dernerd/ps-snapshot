...
Version: 1.0.0
Description: Dieses Plugin ermöglicht es Dir, bei Bedarf schnelle Backup-Snapshots Deiner funktionierenden WordPress-Datenbank zu erstellen. Sie können aus den standardmäßigen WordPress-Tabellen sowie benutzerdefinierten Plugin-Tabellen innerhalb der Datenbankstruktur auswählen. Alle Snapshots werden protokolliert und Sie können den Snapshot nach Bedarf wiederherstellen.
Author: WMS N@W
...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=ps-snapshot', 
	__FILE__, 
	'ps-snapshot' 
);
...
/* Wichtige Dateifunktionen laden (und alles was dazugehört). */
...
if ( ! class_exists( 'PSOURCESnapshot' ) ) {
	...
	class PSOURCESnapshot {
		...
		public $DEBUG = false;
		private $_pagehooks = array(); // Eine Liste unserer verschiedenen Navigationsartikel. Wird beim Einhaken in die Seitenladeaktionen verwendet.
		private $_messages = array(); // Meldung, die während der Formularverarbeitungsschritte für Aktionen zum Hinzufügen, Bearbeiten, Aktualisieren, Löschen und Wiederherstellen gesetzt wird
		private $_settings = array(); // Dies sind globale dynamische Einstellungen, die NICHT als Teil der Konfigurationsoptionen gespeichert werden
		private $_admin_header_error; // Während der Verarbeitung gesetzt enthält Verarbeitungsfehler, die dem Benutzer angezeigt werden
		...
		private function __construct() {
			...
			// Erstellt den Klassen-Autoloader.
			...
			$this->_settings['admin_menu_label'] = __( "Snapshot", SNAPSHOT_I18N_DOMAIN ); // Wird als „option_name“ für die wp_options-Tabelle verwendet
			...
			if ( Snapshot_Helper_Utility::is_pro() ) {
				include_once dirname( __FILE__ ) . '/lib/PSOURCE/Dashboard/psource-dash-notification.php';
			}*/
